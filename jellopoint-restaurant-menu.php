 ...
 * - Dynamic source: renders from canonical jprm_price (v3) with fallbacks
 * - Static source: simple repeater (kept minimal for now)
 * - Loads CSS file + injects inline overrides to beat theme/Elementor styles
 */
class Restaurant_Menu extends Widget_Base {
    ...
    protected function render() {
        // Load base CSS file from plugin, not relative
        if ( defined('JPRM_PLUGIN_URL') ) {
            ...
            // Hard overrides to beat theme/Elementor bullets & large icons
            wp_add_inline_style('jprm-menu', <<<CSS
/* Kill bullets aggressively */
...
/* Icon sizing must beat global img rules */
...
/* Even if img has width/height attributes or parent sets max-width:100% */
.jp-menu__icon[width],
.jp-menu__icon[height]{ width:1.1em !important; height:1.1em !important; }
.elementor-widget img.jp-menu__icon{ max-width:1.1em !important; height:1.1em !important; }
...
/* Two-column price row; reversible order */
    ...
    protected function render_dynamic( array $s ) {
        ...
        foreach ( $items as $post_id ) {
            ...
            if ( $has_v3 ) {
                ...
                // no v3 = nothing to show (we removed legacy reads for cleanliness)
            }
