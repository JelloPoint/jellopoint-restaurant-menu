<?php
/**
 * JelloPoint Restaurant Menu — Render (safe split, Step 3)
 *
 * Pure rendering helpers for 1, 2, and 3 column layouts.
 * No styling, no behavior changes — mirrors the previous widget output.
 *
 * Expects external functions if present:
 * - jprm_render_pricegroup_html()
 * - jprm_render_badges_inline_html()
 * - jprm_infoblocks_render_group()
 * - jprm_infoblocks_partition_by_position() (used earlier in the widget)
 */

if ( ! defined( 'ABSPATH' ) ) { exit; }

/**
 * Small helper to render menu meta (title + description).
 *
 * @param WP_Term|null $term
 * @param bool         $show_title
 * @param bool         $show_desc
 * @param string       $scope 'global'|'col'
 * @return string
 */
 ...
 *   - show_badges, badges_presentation, badges_position
 *   - label_presentation, label_position, label_map, currency_opts
 *   - ib_map
 */
function jprm_render_menu_one_column( array $ctx ) : void {
...
/**
 * Render: 2 columns menu.
 *
 * @param array $ctx (same keys as 1-column + split_mode, split_after_1)
 */
function jprm_render_menu_two_columns( array $ctx ) : void {
	$menu_term           = $ctx['menu_term'] ?? null;
	$show_menu_title     = ! empty( $ctx['show_menu_title'] );
	$show_menu_desc      = ! empty( $ctx['show_menu_desc'] );
	$menu_pos            = $ctx['menu_pos'] ?? 'above_menu';
...
/**
 * Render: 3 columns menu.
 *
 * @param array $ctx (same keys as 2-columns + split_after_2)
 */
function jprm_render_menu_three_columns( array $ctx ) : void {
	$menu_term           = $ctx['menu_term'] ?? null;
	$show_menu_title     = ! empty( $ctx['show_menu_title'] );
	$show_menu_desc      = ! empty( $ctx['show_menu_desc'] );
	$menu_pos            = $ctx['menu_pos'] ?? 'above_menu';
