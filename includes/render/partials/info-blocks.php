...
function jprm_infoblocks_partition_by_position( array $rows ) : array {
    ...
    foreach ( $rows as $r ) {
        ...
        if ( $pos !== 'before_menu' && $pos !== 'after_menu' ) { $pos = 'between_sections'; }
        $out[ $pos ][] = $r;
    }
    return $out;
}
...
/**
 * Legacy: render a group (kept for BC).
 */
function jprm_infoblocks_render_group( array $rows, string $position ) : string {
    ...
    foreach ( $rows as $r ) {
        ...
        if ( $type === 'image' ) {
            ...
            if ( $src ) { $out .= '<div class="' . esc_attr( $cls ) . '"><img src="' . esc_url( $src ) . '" alt="' . esc_attr( $img_alt ) . '" class="jp-infoblock__img" loading="lazy" decoding="async"></div>'; }
            continue;
        }
        if ( $type === 'button' ) {
            ...
            if ( $txt !== '' && $url !== '' ) { $out .= '<p class="' . esc_attr( $cls ) . '"><a class="jp-infoblock__btn" href="' . esc_url( $url ) . '"' . $tgt . $rel . '>' . wp_kses_post( $txt ) . '</a></p>'; }
            continue;
        }
        $html = isset( $r['content_html'] ) ? (string) $r['content_html'] : '';
        if ( $html !== '' ) { $out .= '<div class="' . esc_attr( $cls ) . '">' . wp_kses( $html, $allow ) . '</div>'; }
    }
    $out .= '</div>';
    return $out;
}
...
function jprm_infoblocks_matches_section( array $row, $section_id_or_slug ) : bool {
    ...
    if ( is_numeric( $target ) && (string) (int) $section_id_or_slug === (string) (int) $target ) { return true; }
...
/** Render a list of rows into one container (returns HTML string). */
