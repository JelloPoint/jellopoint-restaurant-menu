...
foreach ( $sections_order as $tid ) {
	...
	if ( isset( $sections_data[$tid] ) ) {
		...
		if ( $t && ! is_wp_error($t) ) $registry[$tid] = ['term'=>$t, 'items'=>[]];
	}
}
...
foreach ( array_keys($registry) as $tid ) {
	...
	while ( $pid ) {
		if ( ! isset($registry[$pid]) ) {
			...
			if ( $pt && ! is_wp_error($pt) ) $registry[$pid] = ['term'=>$pt, 'items'=>[]];
			else break;
		}
...
foreach ( $sections_order as $tid ) {
	...
	while ( $pid && isset($registry[$pid]) ) { $root = $pid; $pid = $__parent_id_of($registry[$pid]['term'] ?? null); }
	if ( ! isset($top_level_seen[$root]) ) { $top_level_seen[$root] = true; $top_level_order[] = $root; }
}

/* ---------------- column split (roots only) ---------------- */
$__index_of_term = function( array $order, int $term_id ) : int { foreach ($order as $i=>$tid){ if((int)$tid===$term_id) return $i; } return -1; };
$__split_sections = function( array $order, int $cols, string $mode, int $id1, int $id2 ) use ($__index_of_term) : array {
	$order = array_values($order); $n = count($order); if ($cols<=1 || $n===0) return [$order];
	if ($mode === 'manual') {
		$idx1 = $__index_of_term($order,$id1); $idx2 = $__index_of_term($order,$id2);
		if ($cols===2) { if ($idx1>=0){$cut=$idx1+1; return [array_slice($order,0,$cut), array_slice($order,$cut)];}
			$cut=(int)ceil($n/2); return [array_slice($order,0,$cut), array_slice($order,$cut)]; }
		if ($idx1>=0 && $idx2>=0 && $idx2>$idx1){$c1=$idx1+1;$c2=$idx2+1; return [array_slice($order,0,$c1), array_slice($order,$c1,$c2-$c1), array_slice($order,$c2)];}
