...
public function render() {
    ...
    // --------------------------------------------------
    // 1. Detect device (for Elementor editor preview)
    // --------------------------------------------------
    $device = 'desktop';
    ...
    if ( class_exists( '\Elementor\Plugin' ) ) {
        try {
            ...
            if ( $plugin->editor->is_edit_mode() ) {
                // Elementor preview mode: 'desktop', 'tablet', 'mobile'
                $mode = method_exists( $plugin->preview, 'get_mode' )
                    ? $plugin->preview->get_mode()
                    : 'desktop';
    ...
    // --------------------------------------------------
    // 2. Global Labels Layout: Desktop / Tablet / Mobile
    // --------------------------------------------------
    $layout_desktop = isset( $s['labels_layout'] )
        ? (string) $s['labels_layout']
        : 'inline';

    $layout_tablet_raw = isset( $s['labels_layout_tablet'] )
        ? (string) $s['labels_layout_tablet']
        : '';

    $layout_mobile_raw = isset( $s['labels_layout_mobile'] )
        ? (string) $s['labels_layout_mobile']
        : '';

    // Inherit: tablet → desktop, mobile → tablet → desktop
    $layout_tablet = $layout_tablet_raw !== '' ? $layout_tablet_raw : $layout_desktop;
    $layout_mobile = $layout_mobile_raw !== '' ? $layout_mobile_raw : $layout_tablet;

    // Also keep the old "global" for backwards-compatibility/inheritance
    // (This is the base layout before per-section overrides.)
    switch ( $device ) {
        case 'tablet':
    ...
    // --------------------------------------------------
    // 4. Query + grouping (unchanged logic)
    // --------------------------------------------------
    ...
    $ctx = [
        ...
        // Global layout base for inheritance
        ...
        'inline_below_sep_enable' => ( ! empty( $s['inline_below_sep_enable'] ) && $s['inline_below_sep_enable'] === 'on' ) ? 'on' : '',
    ...
    if ( file_exists( $overrides_helper ) ) {
        require_once $overrides_helper;
    }
    $icons_helper = dirname( __DIR__ ) . '/helpers/icons.php';
    if ( file_exists( $icons_helper ) ) {
        require_once $icons_helper;
    }
