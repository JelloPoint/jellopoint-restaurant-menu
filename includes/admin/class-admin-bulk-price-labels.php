 ...
 *
 * Admin page under the JelloPoint parent menu. This first implementation:
 ...
 *
 * Later we can plug this into the real price meta structure once confirmed.
 */
final class JPRM_Admin_Bulk_Price_Labels {
    ...
    private static function get_filters_from_request(): array {
        $menu    = isset( $_GET['jprm_menu'] )   ? (int) $_GET['jprm_menu']   : 0;
        $section = isset( $_GET['jprm_section'] )? (int) $_GET['jprm_section']: 0;
    ...
    private static function render_filters( array $filters ): void {
        ...
        // Get menus and sections as dropdown options.
        $menus    = get_terms( [
            'taxonomy'   => 'jprm_menu',
            'hide_empty' => false,
        ] );
        ...
        <form method="get" action="<?php echo esc_url( $action ); ?>">
            <?php
            // Preserve page + parent page query args.
            ...
            <div class="jprm-filters">
                ...
                <div class="field">
                    ...
                    <select id="jprm_section" name="jprm_section">
                        <option value="0"><?php esc_html_e( 'All sections', 'jellopoint-restaurant-menu' ); ?></option>
                        <?php
    ...
    private static function get_flat_price_rows( array $filters ): array {
        ...
        $query_args = [
            ...
            'post_status'    => 'publish',
            'posts_per_page' => 200, // reasonable limit for bulk work; can adjust later.
            'orderby'        => 'title',
            'order'          => 'ASC',
        ];
            ...
            <tbody>
                <?php foreach ( $rows as $r ) : ?>
                    <tr>
                        ...
                        <td>
                            <?php echo $r['menus'] !== '' ? esc_html( $r['menus'] ) : '<span class="jprm-small-meta">' . esc_html__( '(no menu)', 'jellopoint-restaurant-menu' ) . '</span>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
                        </td>
                        <td>
                            <?php echo $r['sections'] !== '' ? esc_html( $r['sections'] ) : '<span class="jprm-small-meta">' . esc_html__( '(no section)', 'jellopoint-restaurant-menu' ) . '</span>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
                        </td>
                        <td>
                            <strong>
                        ...
                        <td>
                            <?php echo $r['price'] !== '' ? esc_html( $r['price'] ) : '<span class="jprm-small-meta">' . esc_html__( '(no price)', 'jellopoint-restaurant-menu' ) . '</span>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
                        </td>
                        <td>
                            <?php
                            if ( $r['label_text'] !== '' ) {
    ...
    private static function handle_bulk_action(): void {
        ...
        if ( ! isset( $_POST['jprm_bulk_price_labels_nonce'] )
            || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['jprm_bulk_price_labels_nonce'] ) ), 'jprm_bulk_price_labels' )
        ) {
            add_settings_error(
        ...
        if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
            error_log( '[JPRM Bulk Price Labels] action=' . $action . ' target_label="' . $target_label . '" rows=' . print_r( $rows, true ) ); // phpcs:ignore
        }
