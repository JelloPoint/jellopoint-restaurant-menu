...
/* --- JPRM: prevent Label cell from overlapping Amount cell --- */
...
/* Lock column widths on both thead/tbody to stabilize layout */
#jprm2-prices-table th:nth-child(1), #jprm2-prices-table td:nth-child(1){ width:32px; }
#jprm2-prices-table th:nth-child(3), #jprm2-prices-table td:nth-child(3){ width:120px; } /* a bit wider for price */
...
/* Strong clipping in the label cell */
#jprm2-prices-table td.label-td{ overflow:hidden !important; position:relative; min-width:0; }
#jprm2-prices-table td.label-td *{ max-width:100%; }

/* Label content must stay within cell */
...
/* Price input must remain visible */
#jprm2-prices-table input.amount{ width:7.5em !important; position:relative; z-index:1; }
...
#postdivrich, #wp-content-media-buttons{display:none!important;}</style>';
    ...
    public static function render_pricing($post){
        ...
        $get_icon_url = function($attachment_id){
            ...
            foreach ($sizes as $s){
                $src = wp_get_attachment_image_src((int)$attachment_id, $s);
        ...
        echo '<style>
            ...
            #jprm-prices-table .small { width:96px; } /* compact amount */
            ...
            #jprm-prices-table input.amount { width:7.2em !important; max-width:100% !important; }
            #jprm-prices-table input.regular-text.small { width:7.2em !important; }
        </style>';
        ...
        echo '<div class="jprm-inline">';
            ...
            echo '<div id="jprm_single_icon_actions" class="jprm-icon-actions" style="'.($lm==='custom' ? '' : 'display:none;').'">';
                echo '<button type="button" class="button-link jprm-single-icon-clear" aria-label="'.esc_attr__('Remove Icon','jellopoint-restaurant-menu').'" style="'.($icon ? '' : 'display:none;').'"><span class="dashicons dashicons-no-alt"></span></button>';
            echo '</div>';
        echo '</div>';
                ...
                <tr>
                    ...
                    <td class="jprm-icon-cell">
                        <div class="jprm-row-icon-preview" title="<?php echo esc_attr__('Change icon','jellopoint-restaurant-menu'); ?>" style="cursor:pointer;">
                            ...
                            <?php else: ?>
                                <span class="jprm-icon-ph-sm" title="<?php echo esc_attr__('Select icon','jellopoint-restaurant-menu'); ?>"><span class="dashicons dashicons-format-image"></span></span>
                            <?php endif; ?>
                        </div>
                    ...
                    <td class="act-col">
                        <button type="button" class="button-link jprm-row-remove" aria-label="<?php echo esc_attr__('Remove row','jellopoint-restaurant-menu'); ?>"><span class="dashicons dashicons-trash"></span></button>
                    </td>
                </tr>
                <?php endforeach; ?>
            </tbody>
        ...
        (function($){
            ...
            $(function(){
                ...
                $('#jprm-row-add').off('click').on('click', function(e){
                    ...
                    $tr.append('<td class="hide-col"><input type="checkbox" class="hide-icon" /></td>');
                    $tr.append('<td class="act-col"><button type="button" class="button-link jprm-row-remove" aria-label="<?php echo esc_js(__('Remove row','jellopoint-restaurant-menu')); ?>"><span class="dashicons dashicons-trash"></span></button></td>');
    ...
    public static function save($post_id, $post){
        ...
        if ($mode === 'single'){
            ...
            if ($lm === 'ref'){
                ...
                $icn  = isset($_POST['jprm_price_label_icon_id']) ? intval($_POST['jprm_price_label_icon_id']) : 0;
                update_post_meta($post_id, 'jprm_price_label_custom', $cus);
                update_post_meta($post_id, 'jprm_price_label_icon_id', $icn);
                delete_post_meta($post_id, 'jprm_price_label_ref');
            }

            if ( ! isset($_POST['jprm_prices']) ) delete_post_meta($post_id, 'jprm_prices');
            ...
            if (is_string($json)){
                ...
                if (json_last_error() === JSON_ERROR_NONE && is_array($rows)){
                    ...
                    foreach ($rows as $r){
                        ...
                        $mode = (($r['label_mode'] ?? 'ref') === 'custom') ? 'custom' : 'ref';
                        $out[] = [
