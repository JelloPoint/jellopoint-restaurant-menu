 ...
 *
 * Renders a compact checkbox list of active badges and saves selected slugs to post meta.
 */
...
class JPRM_MenuItem_Badges_Meta {
	...
	public function render_metabox( $post ) : void {
		...
		// Only active badges are selectable for items.
		$map     = method_exists( $this->store, 'map' ) ? $this->store->map( true ) : [];
		...
		#jprm_item_badges .ph{width:18px;height:18px;display:inline-block}
	...
	public function save_post( $post_id, $post ) : void {
		...
		// Avoid autosaves/revisions meddling
