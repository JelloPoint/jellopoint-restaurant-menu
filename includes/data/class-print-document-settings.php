...
final class Print_Document_Settings {
	...
	public static function defaults() : array {
		return [
			...
			'info_block_layout' => 'beside',
			'info_block_text_color' => '#242424',
			'info_block_background_color' => '#f2f6f7',
			'info_block_alignment' => 'left',
			'info_block_font_size' => 9,
			'info_block_image_width' => 22,
			'info_block_padding' => 3,
			'info_block_spacing' => 4,
			'info_block_border_style' => 'none',
			'info_block_border_width' => 1,
			'info_block_border_color' => '#173f47',
			'info_block_border_radius' => 0,
	...
	public static function sanitize( array $input ) : array {
		...
		$out = [
			...
			'info_block_layout' => self::choice( $input['info_block_layout'] ?? '', [ 'beside', 'stacked' ], 'beside' ),
			'info_block_text_color' => self::color( $input['info_block_text_color'] ?? '', (string) $defaults['info_block_text_color'] ),
			'info_block_background_color' => self::color( $input['info_block_background_color'] ?? '', (string) $defaults['info_block_background_color'] ),
			'info_block_alignment' => self::choice( $input['info_block_alignment'] ?? '', [ 'left', 'center', 'right' ], 'left' ),
			'info_block_font_size' => self::number( $input['info_block_font_size'] ?? null, 6, 24, (float) $defaults['info_block_font_size'] ),
			'info_block_image_width' => self::number( $input['info_block_image_width'] ?? null, 5, 80, (float) $defaults['info_block_image_width'] ),
			'info_block_padding' => self::number( $input['info_block_padding'] ?? null, 0, 20, (float) $defaults['info_block_padding'] ),
			'info_block_spacing' => self::number( $input['info_block_spacing'] ?? null, 0, 30, (float) $defaults['info_block_spacing'] ),
			'info_block_border_style' => self::choice( $input['info_block_border_style'] ?? '', [ 'none', 'solid', 'double', 'dashed' ], 'none' ),
			'info_block_border_width' => self::number( $input['info_block_border_width'] ?? null, 0, 10, (float) $defaults['info_block_border_width'] ),
			'info_block_border_color' => self::color( $input['info_block_border_color'] ?? '', (string) $defaults['info_block_border_color'] ),
			'info_block_border_radius' => self::number( $input['info_block_border_radius'] ?? null, 0, 20, (float) $defaults['info_block_border_radius'] ),
