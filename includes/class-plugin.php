...
final class Plugin {
	private $current_shortcode = array();
	...
	private function __construct() {
		add_action( 'wp_head', array( $this, 'output_alignment_css' ) );

		add_action( 'plugins_loaded', array( $this, 'i18n' ) );
		add_action( 'init', array( $this, 'register_content_model' ) );

		// Admin UI
		add_action( 'admin_menu', array( $this, 'ensure_admin_menu' ) );
		add_action( 'admin_notices', array( $this, 'admin_notice' ) );
		add_filter( 'admin_footer_text', array( $this, 'admin_footer' ) );

		// Editor experience
		add_filter( 'use_block_editor_for_post_type', array( $this, 'disable_block_editor' ), 10, 2 );
		add_action( 'add_meta_boxes', array( $this, 'cleanup_metaboxes' ), 99 );

		add_action(
			'admin_enqueue_scripts',
			function ( $hook ) {
				global $typenow;
				if ( $typenow !== 'jprm_menu_item' ) {
					return; }
...
$(function(){
  ...
  refresh();
		...
		add_action( 'save_post_jprm_menu_item', array( $this, 'save_meta' ), 10, 3 );
		...
		// Shortcode
		add_shortcode( 'jprm_menu', array( $this, 'shortcode_menu' ) );
	}
	...
	public function register_content_model() {
		...
		register_taxonomy(
			'jprm_menu',
			array( 'jprm_menu_item' ),
			array(
				'labels'            => array( 'name' => __( 'Menus', 'jellopoint-restaurant-menu' ) ),
				'public'            => false,
				'show_ui'           => true,
		...
		register_taxonomy(
			...
			array( 'jprm_menu_item' ),
			array(
				'labels'            => array( 'name' => __( 'Sections', 'jellopoint-restaurant-menu' ) ),
				'public'            => false,
				'show_ui'           => true,
				'show_admin_column' => true,
				'hierarchical'      => true,
	...
	public function ensure_admin_menu() {
		add_menu_page( __( 'JelloPoint', 'jellopoint-restaurant-menu' ), __( 'JelloPoint', 'jellopoint-restaurant-menu' ), 'edit_posts', 'jellopoint-root', array( $this, 'render_root' ), 'dashicons-carrot', 56 );
		...
		add_submenu_page( 'jellopoint-root', __( 'Help / Diagnostics', 'jellopoint-restaurant-menu' ), __( 'Help / Diagnostics', 'jellopoint-restaurant-menu' ), 'manage_options', 'jprm-help', array( $this, 'render_help' ) );
	}

	public function render_root() {
		echo '<div class="wrap"><h1>JelloPoint</h1><p>Use the submenu to manage Menu Items.</p></div>'; }
	...
	public function disable_block_editor( $use, $post_type ) {
		return ( 'jprm_menu_item' === $post_type ) ? false : $use; }
	...
	public function register_metaboxes() {
		add_meta_box( 'jprm_item_details', __( 'Menu Item Details', 'jellopoint-restaurant-menu' ), array( $this, 'metabox_html' ), 'jprm_menu_item', 'normal', 'high' ); }
	...
	public function metabox_html( $post ) {
		...
		if ( '' === $visible && 'auto-draft' === $post->post_status ) {
			$visible = 'yes'; }
		?>
		<?php $__on = !empty($multi); ?><div id="jprm-multi-admin" style="<?php echo ($__on?'':'display:none'); ?>"><table class="form-table">
			<tr>
...
echo '</td>'; // JS handled via wp_add_inline_script
			...
			<tr>
				...
				<td>
					<select name="jprm_badge_position" id="jprm_badge_position">
						...
						$cur = $badge_p
						?>
						$badge_p : 'corner-right'; foreach ( $opts as $k=>$label ) printf('<option value="%s"%s>%s</option>', esc_attr($k), selected($cur,$k,false), esc_html($label)); ?>
					</select>
				</td>
			</tr>
			<tr>
	...
	public function save_meta( $post_id, $post, $update ) {
		if ( ! isset( $_POST['jprm_nonce'] ) || ! wp_verify_nonce( $_POST['jprm_nonce'], 'jprm_save_meta' ) ) {
			return;
		}
			...
			if ( is_array( $arr ) ) {
				foreach ( $arr as &$r ) {
					$r = array(
						...
						'label_select' => sanitize_title( $r['label_select'] ?? '' ),
						'label_custom' => sanitize_text_field( $r['label_custom'] ?? '' ),
						'amount' => wp_kses_post( $r['amount'] ?? '' ),
						'hide_icon' => ! empty( $r['hide_icon'] ) ? 1 : 0,
						'order' => intval( $r['order'] ?? 0 ),
					);
				} unset( $r );
		...
		if ( isset( $_POST['jprm_order'] ) ) {
			$order = intval( $_POST['jprm_order'] );
			if ( $post->menu_order !== $order ) {
				wp_update_post(
					array(
						'ID' => $post_id,
						'menu_order' => $order,
	...
	public function admin_footer( $text ) {
		if ( current_user_can( 'manage_options' ) ) {
			$text .= ' | JelloPoint Restaurant Menu v' . esc_html( JPRM_VERSION ) . ' active';
		} return $text; }
	...
	public function enqueue_assets() {
		wp_enqueue_style( 'jprm-frontend', JPRM_PLUGIN_URL . 'assets/css/frontend.css', array(), JPRM_VERSION ); }
	...
	public function shortcode_menu( $atts ) {
		...
		if ( $atts['sections'] ) {
			$tax_query[] = array(
				...
				'field' => 'slug',
				'terms' => array_map( 'trim', explode( ',', $atts['sections'] ) ),
			);
		}
		$meta_query = array();
		if ( intval( $atts['hide_invisible'] ) ) {
			$meta_query[] = array(
				'key' => '_jprm_visible',
				'value' => 'yes',
				'compare' => '=',
		...
		while ( $q->have_posts() ) {
			$q->the_post();
			$this->render_menu_item_from_post( get_the_ID() ); }
		echo '</ul>';
		wp_reset_postdata();
	...
	public function render_menu_item_from_post( $post_id ) {
		...
		if ( $admin_rows_json ) {
			$arr = json_decode( $admin_rows_json, true );
			if ( is_array( $arr ) ) {
				$preset = function_exists( 'jprm_get_price_label_full_map' ) ? jprm_get_price_label_full_map() : array();
				usort(
					$arr,
					function ( $a, $b ) {
				...
				foreach ( $arr as $r ) {
					...
					$lbl = ( $sel === 'custom' || $sel === '' ) ? ( $r['label_custom'] ?? '' ) : ( $preset[ $sel ]['label'] ?? '' );
					$amt = $r['amount'] ?? '';
					if ( $lbl === '' && $amt === '' ) {
						continue;
					} $rows[] = array(
		...
		if ( empty( $rows ) && $multi_rows_raw ) {
			foreach ( preg_split( '/\r?\n/', $multi_rows_raw ) as $line ) {
				...
				$parts = explode( '|', $line, 2 );
				$label = trim( $parts[0] );
				$pval = isset( $parts[1] ) ? trim( $parts[1] ) : '';
				$rows[] = array(
					'label' => $label,
					'price' => $pval,
				);
		...
		$img  = get_the_post_thumbnail( $post_id, 'thumbnail', array( 'class' => 'attachment-thumbnail size-thumbnail' ) );
		...
		if ( $img ) {
			echo '<div class="jp-menu__media">' . $img . '</div>';
		}
		...
		if ( $multi && ! empty( $rows ) ) {
			...
			foreach ( $rows as $r ) {
				$icon_html = '';
				if ( ! empty( $r['icon_id'] ) ) {
					$icon_html = '<span class="jp-price-icon" aria-hidden="true">' . wp_get_attachment_image( intval( $r['icon_id'] ), 'thumbnail', false, array( 'alt' => '' ) ) . '</span>'; }
			...
			if ( $price ) {
				...
				$label_presentation = isset( $this->current_shortcode['label_presentation'] ) ? $this->current_shortcode['label_presentation'] : 'text';
				$wrapper_class = 'jp-menu__price-row ' . ( $row_order === 'price_left' ? 'jp-order--price-left' : 'jp-order--label-left' );
				$preset_full = function_exists( 'jprm_get_price_label_full_map' ) ? jprm_get_price_label_full_map() : array();
				...
				? '<span class="jp-price-icon" aria-hidden="true">' . wp_get_attachment_image( intval( $preset_full[ $slug_guess ]['icon_id'] ), 'thumbnail', false, array( 'alt' => '' ) ) . '</span>'
				: '';
		...
		echo '</div>';
		if ( $sep ) {
			echo '<div class="jp-menu__separator" aria-hidden="true"></div>';
		}
		echo '</li>';
	}
